---
{"dg-publish":true,"permalink":"/data-driven-customization/castle-oblivion-data/"}
---
...
Floor Type JSONs are in the `castle_oblivion/floor_type` folder. It is possible to add new Floor Types and a new card item through KubeJS or any item with the `kingdomkeys:world_card` component with a string of the registry name of the floor_type.
...
* `(string) biome_colours`: The biome to get the colours for the grass, water, foliage and sky colour from for the rooms on this floor. The sky colour is used for the wall colours unless `use_fog_colour` is true or if a [[data-driven customization/Castle Oblivion Data#Room Type\|Room Type]] overrides it.
* `[optional] (string) music`: The music to play while in the rooms can be overridden by the [[data-driven customization/Castle Oblivion Data#Room Type\|Room Type]].
...
Room Type JSONs are in the `castle_oblivion/room_type` folder. It is possible to add new Room Types however adding a new card that uses that type is supported through KubeJS.
Here is a breakdown of the JSON file structure:
...
* `[optional] (object) enemies`:  This contains the properties for rooms that have random enemy spawns.
	...
	* `[optional] (tag string) regular_enemies`: The tag for the enemy spawning pool if this is not set `regular_enemies` from the [[data-driven customization/Castle Oblivion Data#Floor Type\|Floor Type]] is used.
	* `[optional] (tag string) strong_enemies`: Currently not implemented.
...
* `[optional] (array modifier) modifiers`: The modifiers for the room see [[data-driven customization/Castle Oblivion Data#Room Modifier\|Room Modifier]] for how these work.
* `[optional] (array string) compatible`: The [[data-driven customization/Castle Oblivion Data#Floor Type\|Floor Type]] this room is compatible with should be an existing floor type such as `kingdomkeys:plains`.
...
* `[optional] (string) encounter`: The [[data-driven customization/Castle Oblivion Data#Room Encounter\|Room Encounter]] for the room.
* `[optional] (object) treasure`: The treasure to spawn in the room structure requires Treasure Chests or mob spawn points as a fallback to work. 
	* `(string) treasure`: The loot table for the treasure.
	* `(int) count`: The number of treasure chests to spawn.
	* `[optional] (int) trapped_count`: The number of trapped treasure chests to spawn, these are ones that spawn an enemy when you try to open them.
	* `[optional] (string) enemey`: The tag for the enemies to spawn from trapped treasure chests.
...
* `(enum string array) categories`: This is a list of the compatible categories of [[data-driven customization/Castle Oblivion Data#Room Type\|Room Type]]. Possible values are "ENEMY", "STATUS", "BOUNTY" "SPECIAL", "ENCOUNTER", "ANY".
...
Room Encounter JSONs are in the `castle_oblivion/room_encounter` custom ones can simply be made by making a new file and setting the encounter for a [[data-driven customization/Castle Oblivion Data#Room Type\|Room Type]] with the new encounter.
Here is a breakdown of the JSON file structure:
* `(object) encounter`:  The definition for the encounter.
	* `(string) type`: The type of encounter this determines the rest of the options for the encounter see [[data-driven customization/Castle Oblivion Data#Encounter Types\|Encounter Types]].
...
* `(object array) waves`: The list of waves.
	...
	* `[optional] (modifier array) modifiers`:  The modifiers for the wave see [[data-driven customization/Castle Oblivion Data#Room Modifier\|Room Modifier]] for how these work. Modifiers that do something when the room is generated will not do anything.
...
> ```
# Room Modifier
Room Modifiers are quite versatile objects that can be added to a [[data-driven customization/Castle Oblivion Data#Room Type\|Room Type]], [[data-driven customization/Castle Oblivion Data#Floor Type\|Floor Type]] and [[data-driven customization/Castle Oblivion Data#Wave\|Wave]] Encounters. They can do things when a room is entered and exited, every tick while players are in a room, when a room is generated and when a mob spawns in a room. Each type of modifier has its own options for the JSON.
## Effect
...
## Drop
The drop type `kingdomkeys:drop` modifies the items dropped by mobs in rooms on death.

Here is a breakdown of the JSON structure:
* `(object) item`: The item to drop this is an ItemStack so it can have components and count, etc.
* `[optional] (bool) replace_card`: If set to true the item will drop instead of the map cards that usually drop by default it is false.
* `[optional] (int) chance`:  The percentage chance for the item to drop by default it is 100, if `replace_card` is true and the chance is less than 100 when the specified item doesn't drop it will drop the card instead.

>[!example] Example: roulette_room.json
>```json
>"modifiers": [
>  {
>    "type": "kingdomkeys:drop",
>    "chance": 50,
>    "item": {
> 	   "count": 1,
> 	   "id": "kingdomkeys:roulette_bonus"
>    },
>    "replace_card": true
>]
>```
>This makes Roulette Bonuses drop with a 50% chance and otherwise drops the regular Map Cards.
